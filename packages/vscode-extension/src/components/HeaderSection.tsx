...
const HeaderSection: React.FC<HeaderSectionProps> = ({ task, copyToClipboard }) => {
  return (
    <div className="header-row">
      <div className="header-left">
        <h1>
          Edit Task <span id="headerTicketId">#{task.id}</span>
          <button
      ...
      <div className="header-metadata">
        <div className="header-metadata-item">
          ...
          <div className="header-metadata-value">{formatDate(task.created_at)}</div>
        </div>
        <div className="header-metadata-item">
          <span className="header-metadata-label">Updated:</span>
          <div className="header-metadata-value">{formatDate(task.updated_at)}</div>
        </div>
        {task.closed_at && (
          <div className="header-metadata-item">
            <span className="header-metadata-label">Closed:</span>
            <div className="header-metadata-value">{formatDate(task.closed_at)}</div>
          </div>
        )}
      </div>
    </div>
